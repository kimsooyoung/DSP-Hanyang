...
F2H = figure(3); % Vital Signal plot�� ���� figure
...
%% 
            ...
            if ( (DataCursor1==1)&&(ResetCounter1==0) )
                ...
                VitalSignFreq_Stack = zeros(DefaultWindowL_Time*FPS,1);
            end
...
%                     
            ...
            if DataCursor1 > DefaultWindowL_Time * FPS
                VitalSignBeforeFilter = B_LowPass1(DataCursor1-199:DataCursor1,HP);
                HP_FFT = abs(fft(VitalSignBeforeFilter, 1024));
                [~,VitalSignFreq] = max(HP_FFT);
                VitalSignFreq = VitalSignFreq / 1024 * 20 * 60;
                VitalSignFreq_Stack(DataCursor_ForHP) = VitalSignFreq;
                set(VitalSign_F, 'XData', 1:1:200, 'YData', VitalSignBeforeFilter);
                th2.String = ['Vital Signal Frequency: ' num2str(VitalSignFreq) ];  
            end
        %%
