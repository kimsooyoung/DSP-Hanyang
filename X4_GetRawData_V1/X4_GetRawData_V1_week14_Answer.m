...
% F2H = figure(3); 
% subplot(2,1,1)
% VitalSign_F = plot(0,0);% Vital Signal plot�� ���� figure
% axis auto
% 
% subplot(2,1,2)
% VitalSign_FFT_F = plot(0,0);% Vital Signal�� ���ļ� �м� plot�� ���� figure
% xlim([0 60])
% ylim([0 inf])

% th2 = title(''); % Vital Signal Frequency ���� ǥ��
...
%                     ylim([-0.1 2]);
            ...
            if ( (DataCursor1==1)&&(ResetCounter1==0) )
                ...
                %% CFAR Variables
                CFARthres = zeros(DataLengthCorrected1,1);
                GuardInterval = 30;
                CellInterval = 100;
                ThresholdAlpha = 4;
            end
...
%             VitalSignBeforeFilter = B_LowPass1(DataCursor1-DefaultWindowL_Time*FPS+1:DataCursor1,HP);  % B_LowPass1���� ȣ����ȣ �ֱ� 10�ʰ� �̱�
%             [~,VitalSignFreq] = max(abs(fft(VitalSignBeforeFilter,2^12)));                             % VitalSignBeforeFilter���� fft�� ���� ȣ���� �����ϱ�, max �Լ� ���
%             VitalSignFreq = VitalSignFreq/2^12 * 20 * 60;                                              % ������ ȣ����ȣ index�� ���� �д� ȣ������ ��ȯ�ϱ�
%             set(VitalSign_F, 'XData', 1:length(VitalSignBeforeFilter), 'YData', VitalSignBeforeFilter);% set�Լ��� ����Ͽ� VitalSign_F Figure�� ȣ����ȣ �ǽð����� plot �ϱ�
%             th2.String = ['Vital Signal Frequency: ' num2str(VitalSignFreq) ];                         % VitalSign_F Figure�� Title�� �д� ȣ���� �ǽð����� ����ϱ�
%             
%             set(VitalSign_FFT_F, 'XData', (1:2^12)/2^12 * 20 * 60, 'YData', abs(fft(VitalSignBeforeFilter,2^12))); % VitalSign_FFT_F�� VitalSignal�� FFT ��� �ǽð����� ���
%             
%             end
            
             %% CFAR Algorithm
            for i=1:1:DataLengthCorrected1
                if i<=GuardInterval
                    CFARthres(i,1) = ThresholdAlpha*mean(Hilbert_temp(i+GuardInterval:i+GuardInterval+CellInterval));
                elseif (i>GuardInterval) && (i<=CellInterval+GuardInterval)
                    CFARthres(i,1) = ThresholdAlpha*mean([Hilbert_temp(1:i-GuardInterval) Hilbert_temp(i+GuardInterval:i+GuardInterval+CellInterval)]);
                elseif (i>CellInterval+GuardInterval) && (i<=DataLengthCorrected1-CellInterval-GuardInterval)
                    CFARthres(i,1) = ThresholdAlpha*mean([Hilbert_temp(i-GuardInterval-CellInterval:i-GuardInterval) Hilbert_temp(i+GuardInterval:i+GuardInterval+CellInterval)]);
                elseif (i>DataLengthCorrected1-CellInterval-GuardInterval) && (i<=DataLengthCorrected1-GuardInterval)
                    CFARthres(i,1) = ThresholdAlpha*mean([Hilbert_temp(i-GuardInterval-CellInterval:i-GuardInterval) Hilbert_temp(i+GuardInterval:end)]);
                else
                    CFARthres(i,1) = ThresholdAlpha*mean(Hilbert_temp(i-GuardInterval-CellInterval:i-GuardInterval));
                end
        ...
        set(Hilbert_F, 'XData', (1:length(Hilbert_temp)), 'YData', Hilbert_temp);
        set(CFAR_F, 'XData', (1:length(CFARthres)), 'YData', CFARthres);
        drawnow;
    end
    ...
    figure;
    plot(VitalSignBeforeFilter);
    hold on
    
    VitalSignAfterFilter = lowpass(VitalSignBeforeFilter, 60/60, 20);
    plot(VitalSignAfterFilter);
