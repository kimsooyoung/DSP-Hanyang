classdef DataReader
    ...
    methods
        ...
        function status = set_filter(this, data_types)
            ...
            %
            % Input:
            % - data_types Specifies the filter as an array that consists of a combination of DataType flags.
            for i = 1:length(data_types)
                status = this.dataRecorderInterface.data_reader_set_filter(this.dataReader_instance, data_types(i));
            end
        end
        ...
        function [data_types,status] = get_filter(this)
            ...
            data_types_sum = double(dataTypesPtr.Value);
            data_types = ModuleConnector.DataReader.convertDataTypesSumToArray(data_types_sum);
            clear dataTypesPtr;
        end
        ...
        function [data_types,status] = get_data_types(this)
            %GET_SIZE This function returns an array of all data types included in the recording.
            dataTypesPtr = libpointer('uint32Ptr',0);
    ...
    methods (Static)
        function data_types_array = convertDataTypesSumToArray(data_types_sum)
            i = 13;
            ctr = 1;
            while i >= 0
                if data_types_sum - 2^i >= 0
                    data_types_array(ctr) = 2^i;
                    data_types_sum = data_types_sum - 2^i;
                    ctr = ctr+1;
                end
                i = i-1;
            end            
    ...
    methods
        ...
        %% Constructor
        function dr = DataReader(mc)
            % Constructor
            
            % Get DataRecorderInterface layer one wrapper class.
            dr.dataRecorderInterface = ModuleConnector.DataRecorderInterface(mc.lib_name);
            
            % Get DataReader libpointer instance.
            dr.dataReader_instance = calllib(mc.lib_name,'data_reader_create');
