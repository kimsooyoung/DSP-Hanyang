classdef RadarInterface
    ...
    %===================================================================================
        ...
        %
        function status = xep_x4driver_set_dac_step( this, xep_instance, dac_step)
            status = calllib(this.lib_name,'xep_x4driver_set_dac_step', xep_instance, uint8(dac_step));
            assert(status==0,'xep_x4driver_set_dac_step:libraryStatusFailed',strcat('Call returns status=',num2str(status)));
        end
        %
...
%            status=calllib(this.lib_name,'nva_system_run_test',xep_instance,uint8(test_code),data_lengthPtr,dataPtr);
    ...
    %===================================================================================
        ...
        function status = set_iopin_control( this, X4M_instance, pin_id, pin_setup, pin_feature)
            ...
            status = calllib(this.lib_name,'nva_set_iopin_control',X4M_instance, uint32(pin_id), uint32(pin_setup), uint32(pin_feature));
        ...
        function status = get_iopin_control(this, X4M_instance,pin_id,pin_setupPtr,pin_featurePtr)
            % Return setting
            status = calllib(this.lib_name,'nva_get_iopin_control',X4M_instance,uint32(pin_id),pin_setupPtr,pin_featurePtr);
        ...
        function status = read_message_baseband_iq(this, X4M_instance,frame_counterPtr, num_binsPtr, bin_lengthPtr, sample_frequencyPtr,carrier_frequencyPtr, range_offsetPtr, i_dataPtr, q_dataPtr, max_length)
         ...
         % 1.1.0 / 1.0.0
        function int = peek_message_pulsedoppler(this, X4M_instance)
            % Return number of messages available.
            int = calllib(this.lib_name,'nva_peek_message_pulsedoppler',X4M_instance);
            %int peek_message_pulsedoppler()
        end
...
%         % TODO: TBD
%         function int = do_generate_noisemap(this, X4M300_instance)
%         %This command starts the module-self calibration process (that also is performed at power up with the default module set-up). The noise map calibration result is stored to non-volatile memory and is used the next time the module powers up.
%         % Running this command will stop whatever mode of operation that was preformed prior to the reception of the command.
%         % The module will be in idle state after a calibrate command. the execute_profile command must be run in order to re-start the module
%         int do_generate_noisemap()
%         end
%         
%         % TODO: TBD
%         function int = set_noisemap(this, X4M300_instance, data_length, dataPtr)
%             %This command allows the user to load a predefined noisemap to the module. It is stored in flash.
%             int set_noisemap(uint32_t data_length, uint8_t * data)
%         end
%         % TODO: TBD
%         function int = get_noisemap_control(this, X4M300_instance, noisemap_controlPtr)
%             %
%             int get_noisemap_control(uint32_t * noisemap_control)
%         end
%         % TODO: TBD
%         function int = get_noisemap(this, X4M300_instance, data_lengthPtr, dataPtr)
%         % This command returns the stored noisemap.
%         int get_noisemap(uint32_t * data_length, uint8_t ** data)
%         end
%         % TODO: TBD
%         function int = peek_message_noisemap(this, X4M300_instance)
%             % Return number of messages available.
%             %int peek_message_noisemap()
%         end
%         % TODO: TBD
%         function int = read_message_noisemap(TBD)
%             %int read_message_noisemap(TBD)
%         end
